<template>
  <div
    class="flex items-center justify-center min-h-screen bg-gradient-to-br from-gray-100 via-white to-gray-50 px-4"
  >
    <Card
    ...
    >
      ...
      <CardContent>
        <form @submit.prevent="login" class="space-y-5">
          ...
          <Button
            type="submit"
            class="w-full rounded-xl hover:scale-[1.02] transition-transform duration-150"
          >
            Sign In
          </Button>
        </form>
...
const login = async () => {
  try {
    ...
    if (res.data.statusCode === 200 || res.data.statusCode === "200") {
      ...
      // If you updated useAuth to store user:
      // setUser(res.data.user);
      router.push("/explore");
    } else {
      alert(res.data.statusMessage || "Login failed");
    }
  } catch (e) {
